...
void run_scaffold(IR_state_t *IRstate, char **line)
{
   ...
   else if (match_second_word(line, "TERMINATE_WITH_CODEFREE"))
   {
      ...
      full_free((char***)IRstate->vars[var_dest]);
      EIR_data_t *gd = get_eba_arg(eba_args[IRstate->w_thread], 1);
   ...
   else if (match_second_word(line, "PTHREAD_SPAWN_HEAVY"))
   {
      ...
      EIR_data_t *gd_old = get_eba_arg(eba_args[IRstate->w_thread], 1);
      ...
      EIR_data_t *gd_new = malloc(sizeof(EIR_data_t));
      // same pointers to op loaders and frargs. But new stored arg and thread.
      memcpy(gd_new, gd_old, sizeof(EIR_data_t));
