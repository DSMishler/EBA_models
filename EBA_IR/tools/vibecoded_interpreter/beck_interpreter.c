...
void* parse_var_buf(char *word, IR_state_t *IRstate)
{
   ...
   {
      char *myval = word+1;
      int i = strchr(myval,'"')-myval;
      retval = (char *)malloc(i+1);
      strncpy(retval,word+1,i);
      ((char*)retval)[i] = 0;
   }
   else
   {
...
void run_bufreq(IR_state_t *IRstate, char **line)
{
   ...
   else if (match_second_word(line, "ALLOC_LITERAL"))
   {
      ...
      IRstate->vars[var_dest] = (int64_t) newbuf;
      IRstate->len = lit_allocation_len;
   }
...
void run_system(IR_state_t *IRstate, char **line)
{
      assert(confirm_first_word(line, "SYSTEM"));

      void *in_buf = parse_var_buf(line[1], IRstate);

//      printf("boutta call system starting at 0x%lx\n", (uint64_t)adr);
      system(in_buf);

     buf_free_if_shorthand(in_buf, line[1]);
     IRstate->next_line += 1;
}

void run_strstr(IR_state_t *IRstate, char **line)
{
      assert(confirm_first_word(line, "STRSTR"));

      void *in_buf = parse_var_buf(line[1], IRstate);
      void *pat_buf = parse_var_buf(line[2], IRstate);


//      fprintf(stderr, "boutta call strstr %s %s\n", (char *)in_buf, (char *)pat_buf);
//      fprintf(stderr, "first char: %c\n", *strstr(in_buf, pat_buf));

        size_t haystack_len = strlen(in_buf);
        size_t needle_len = strlen(pat_buf);
	
        for (size_t i = 0; i <= haystack_len - needle_len; i++) {
        size_t j;

        // Check for a match starting from index i
        for (j = 0; j < needle_len; j++) {
            if (((char *)in_buf)[i + j] != ((char *)pat_buf)[j]) {
                break; // Mismatch found, move to the next position in haystack
            }
        }

        // If the inner loop completed without a break, the entire needle was found
        if (j == needle_len) {
              fprintf(stderr, "first char: %c\n", ((char *)in_buf)[i]);	
	      buf_free_if_shorthand(in_buf, line[1]);
              buf_free_if_shorthand(pat_buf, line[2]);
              IRstate->next_line += 1;
	      return;
        }
   }
...
void run_dlopen(IR_state_t *IRstate, char **line)
{
      void *handle, (*codeptr)(void *);
      char soname[100], strsymb[100], *argbuf;
      
      assert(confirm_first_word(line, "DLOPEN"));

      void *in_buf = parse_var_buf(line[1], IRstate);
//      void *arg_buf = parse_var_buf(line[2], IRstate);

     strcpy (soname, "./");
     strcat(soname, in_buf);
     strcat(soname, ".so");
//     fprintf(stderr, "boutta call dlopen %s\n", soname);
     handle = dlopen(soname, RTLD_NOW);
//     fprintf(stderr, "dlerror %s\n", dlerror());
     strcpy(strsymb, in_buf);
//     fprintf(stderr, "boutta call dlsym %s\n", (char *)strsymb);
     codeptr = dlsym(handle, strsymb);
//     fprintf(stderr, "boutta call dlsym arg %d\n", (int)codeptr);
//     fprintf(stderr, "dlerror %s\n", dlerror());
     argbuf = "hello world";
     (*codeptr)(argbuf);
//     fprintf(stderr, "%s\n", argbuf);
...
void run_line(IR_state_t *IRstate, char **line)
{
   ...
   else if (samestr(line[0], "SYSTEM"))
   {
      run_system(IRstate, line);
   }
   else if (samestr(line[0], "DLOPEN"))
   {
      run_dlopen(IRstate, line);
   }
   else if (samestr(line[0], "STRSTR"))
   {
      run_strstr(IRstate, line);
   }
   else
   {
